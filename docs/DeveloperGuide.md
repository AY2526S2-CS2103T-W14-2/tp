...
7. **Implement checking for the inclusion of real dates in `schedule` and `reschedule` command:** The current implementation of the `schedule` and `reschedule` commands allow for no real dates or not all days of the week entered to be included in the date range. We plan to implement checks to ensure that the all days of the week entered by the user are included on at least one date in the date range and if this criterion is not satisfied, an error message will be shown to the user.
    * Example 1: `schedule st/2026-04-13 ed/2026-04-13 tm/12:00 d/5` is currently accepted by ServeMate but the date range in this command does not contain an actual date because `2026-04-13` represents a date that is on a Monday and not a Tuesday.
